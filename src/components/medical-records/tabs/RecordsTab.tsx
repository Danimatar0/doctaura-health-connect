 ...
 *
 * Displays medical records (visits, lab tests, imaging, etc.)
 * Refactored from the original MedicalRecords page.
 */
...
const RecordsTab = ({ records, isLoading = false }: RecordsTabProps) => {
  // Helper functions
        ...
        <Card className="shadow-soft hover:shadow-hover transition-smooth cursor-pointer">
          <CardContent className="pt-6">
            <div className="flex items-start gap-4">
              <div
                className={`w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0 ${getRecordColor(record.type)}`}
              >
      ...
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <div className="flex items-center gap-3">
            <div
              className={`w-12 h-12 rounded-xl flex items-center justify-center ${getRecordColor(record.type)}`}
            >
              {getRecordIcon(record.type)}
            </div>
            <div>
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-4">
        <Card className="shadow-soft">
          <CardContent className="pt-6">
            <div className="text-center">
              <div className="bg-primary/10 text-primary w-10 h-10 rounded-xl flex items-center justify-center mx-auto mb-2">
        ...
        <Card className="shadow-soft">
          <CardContent className="pt-6">
            <div className="text-center">
              <div className="bg-purple-500/10 text-purple-500 w-10 h-10 rounded-xl flex items-center justify-center mx-auto mb-2">
                <FlaskConical className="h-5 w-5" />
              </div>
              <p className="text-xl font-bold">{recordsByType.lab.length}</p>
        ...
        <Card className="shadow-soft">
          <CardContent className="pt-6">
            <div className="text-center">
              <div className="bg-green-500/10 text-green-500 w-10 h-10 rounded-xl flex items-center justify-center mx-auto mb-2">
                <Scan className="h-5 w-5" />
              </div>
              <p className="text-xl font-bold">{recordsByType.imaging.length}</p>
        ...
        <Card className="shadow-soft">
          <CardContent className="pt-6">
            <div className="text-center">
              <div className="bg-blue-500/10 text-blue-500 w-10 h-10 rounded-xl flex items-center justify-center mx-auto mb-2">
                <Activity className="h-5 w-5" />
              </div>
              <p className="text-xl font-bold">{recordsByType.visit.length}</p>
        ...
        <Card className="shadow-soft">
          <CardContent className="pt-6">
            <div className="text-center">
              <div className="bg-orange-500/10 text-orange-500 w-10 h-10 rounded-xl flex items-center justify-center mx-auto mb-2">
                <Syringe className="h-5 w-5" />
              </div>
              <p className="text-xl font-bold">{recordsByType.vaccination.length}</p>
        ...
        <CardContent>
          <Tabs defaultValue="all">
            <TabsList className="mb-6 flex-wrap h-auto">
              ...
              <TabsTrigger value="lab">Lab ({recordsByType.lab.length})</TabsTrigger>
              <TabsTrigger value="imaging">Imaging ({recordsByType.imaging.length})</TabsTrigger>
              <TabsTrigger value="vaccination">
                Vaccines ({recordsByType.vaccination.length})
              </TabsTrigger>
              <TabsTrigger value="procedure">
                Procedures ({recordsByType.procedure.length})
              </TabsTrigger>
            </TabsList>

            {Object.entries(recordsByType).map(([key, typeRecords]) => (
              <TabsContent key={key} value={key} className="space-y-4">
                {typeRecords.length === 0 ? (
                  ...
                  typeRecords.map((record) => <RecordCard key={record.id} record={record} />)
                )}
              </TabsContent>
            ))}
          </Tabs>
